<template>
  <!-- 设备和网络功能（4 卡片） -->
  <section class="bg-white py-14 md:py-[120px]">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <p
        class="font-pingfang font-normal text-left md:text-center text-[16px] md:text-[18px] leading-[26px] md:leading-[26px] tracking-normal text-[#93959C] max-w-[560px] mx-auto mb-[46px] md:mb-[90px]">
      ...
      <div class="hidden md:grid grid-cols-2 gap-8 max-w-6xl mx-auto">
        <div v-for="item in deviceNetworkFeatures" :key="item.title"
          class="bg-white rounded-[12px]  shadow-[0_2px_24px_0_rgba(0,0,0,0.096)] border-none p-8 flex gap-6 items-center max-w-[90vw]">
          <SvgIcon :name="item.icon" class="w-[100px] h-[100px] lg:w-[160px] lg:h-[160px] object-contain flex-shrink-0"
            alt="图标" />
          <div class="flex-1">
            <h4 class="font-pingfang font-medium text-[28px] leading-[36px] tracking-normal text-[#111827] mb-4">
              {{ item.title }}
            </h4>
            <p class="font-pingfang font-normal text-[18px] leading-[26px] tracking-normal text-[#626671]">
              {{ item.desc }}
            </p>
          </div>
        </div>
      ...
      <div class="md:hidden">
        <div ref="deviceScrollEl"
          ...
          <div v-for="(item, idx) in deviceNetworkFeatures" :key="item.title" :ref="(el) => setDeviceCardRef(el, idx)"
        ...
        <div class="flex justify-center gap-2 mt-2">
          <span v-for="(_, i) in deviceNetworkFeatures" :key="i" class="w-2 h-2 rounded-full transition-colors"
