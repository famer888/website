<template>
  <div class="bg-white">
    ...
    >
      <!-- 顶部大图：用 img + object-cover，自适应不同屏幕（1920x560） -->
      <img
        :src="img_t1dsp6x"
        alt="T1 DSP"
      ...
      <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
        <div class="h-full flex items-center">
          <div
            class="w-full flex flex-col md:flex-row items-center gap-8 lg:gap-12"
          >
            <!-- 文案 -->
            <div class="w-full md:w-1/2 text-white">
              <h1
                class="font-pingfang font-medium text-[32px] sm:text-[40px] md:text-[80px] leading-[44px] sm:leading-[54px] md:leading-[110px] tracking-normal mb-4"
              >
                什么是<br />
                T1 DSP?
              </h1>
              <!-- 一条线 -->
              <div
                class="w-[120px] sm:w-[160px] md:w-[240px] h-[1px] bg-white/60 mb-4"
              ></div>

              <p
                class="font-pingfang font-normal text-[14px] sm:text-[16px] md:text-[24px] leading-[22px] sm:leading-[26px] tracking-normal text-white/80"
              >
                广告投放站点
              </p>
            </div>

            <!-- 占位列：保证 PC 端文案保持在左半边（背景图在右侧） -->
            <div class="hidden md:block md:w-1/2" aria-hidden="true" />
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="bg-white py-10 sm:py-14 lg:py-16">
      <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2
          class="font-pingfang font-medium text-[28px] sm:text-[32px] md:text-[36px] leading-[40px] sm:leading-[46px] md:leading-[52px] tracking-normal text-[#111827] mb-4"
        >
          T1 DSP
        </h2>
        <p
          class="font-pingfang font-normal text-[12px] sm:text-[14px] md:text-[16px] leading-[20px] sm:leading-[22px] md:leading-[26px] tracking-normal text-[#A2A3A5] max-w-[760px] mx-auto mb-6"
        >
          T1 DSP
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row items-center gap-10 lg:gap-16">
          ...
          <div class="w-full md:w-1/2">
            ...
            <p
              class="font-pingfang font-normal text-[12px] sm:text-[14px] md:text-[16px] leading-[20px] sm:leading-[22px] md:leading-[26px] tracking-normal text-[#A2A3A5] max-w-[560px]"
            >
              通过投放我们的网站受众平台，您可以访问我们的优质广告资源，触达更多潜在客户，并制定更具创新性的营销策略。评估您的广告系列效果，了解哪些系列能够真正推动线上线下业务增长，创造实际价值。
            </p>
          </div>
          ...
          <div class="w-full md:w-1/2 flex justify-center md:justify-end">
            <img
              :src="img_dsp7"
              alt="拓展广告业务"
              class="w-full h-auto max-w-[560px] object-contain"
            />
          </div>
        </div>
      </div>
    ...
    <section class="bg-white py-12 sm:py-14 lg:py-16">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <h3
          class="font-pingfang font-medium text-[22px] sm:text-[24px] md:text-[28px] leading-[32px] sm:leading-[36px] md:leading-[40px] tracking-normal text-[#111827] text-center mb-8"
        >
          DSP常见问题解答
        </h3>
        ...
        <div class="border-t border-[#E6ECF5]">
          ...
          >
            ...
            <div v-if="openFaqIndex === idx" class="pb-4">
              <p
                class="font-pingfang font-normal text-[12px] sm:text-[14px] leading-[20px] sm:leading-[22px] tracking-normal text-[#A2A3A5]"
              >
                {{ item.a }}
              </p>
            </div>
...
const faqItems = [
  {
    q: "什么是需求方平台(DSP)?",
    a: "DSP 是帮助广告主通过程序化方式购买广告资源的平台，可在多个站点与位置高效投放并进行数据分析与优化。",
  },
  {
    q: "什么是程序化广告?",
    a: "程序化广告是通过自动化技术与数据驱动策略完成广告购买与展示的方式，可更快速、精准地触达目标受众。",
  },
  {
    q: "我可以在 T1 DSP 上购买哪些广告?",
    a: "您可以购买展示、视频等多种广告形式，并结合站点、位置、受众等维度进行组合投放。",
  },
  {
    q: "我该如何管理我的营销活动?",
    a: "从目标与预算开始，上传素材并选择站点与位置，投放后根据关键指标（展示、点击、转化等）持续优化出价与定向。",
  },
];
...
const digitalScrollEl = ref(null);
const uniqueCardEls = ref([]);
const digitalCardEls = ref([]);
const activeUniqueIndex = ref(0);
const activeDigitalIndex = ref(0);

const setUniqueCardRef = (el, idx) => {
  if (el) uniqueCardEls.value[idx] = el;
};

const setDigitalCardRef = (el, idx) => {
  if (el) digitalCardEls.value[idx] = el;
};
...
const cardSections = [
  {
    ...
    bgColor: "bg-[#F5F7FB]",
    scrollRef: uniqueScrollEl,
    cardRefs: uniqueCardEls,
    activeIndex: activeUniqueIndex,
  ...
  },
  {
    ...
    items: digitalAdFeatures,
    bgColor: "bg-white",
    scrollRef: digitalScrollEl,
    cardRefs: digitalCardEls,
