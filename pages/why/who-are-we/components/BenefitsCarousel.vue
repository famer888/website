<template>
	<div class="w-full">
		<!-- 标题 -->
		<div class="text-left md:text-center mb-[36px] md:mb-[48px]">
			<h2
		...
		<div class="hidden md:grid grid-cols-4 gap-4 mb-8 max-w-5xl mx-auto">
			<div v-for="(card, index) in cards" :key="index" @click="activeIndex = index"
				class="cursor-pointer transition-all duration-300 p-6 rounded-lg border" :class="activeIndex === index
					? 'bg-[#3A82F9] text-white border-[#3A82F9]'
					: 'bg-white text-gray-900 border-gray-200 hover:border-[#3A82F9]'">
				<h3 class="font-bold text-lg mb-3" :class="activeIndex === index ? 'text-white' : 'text-[#626671]'">{{
					card.title }}</h3>
					...
					<div v-for="(card, index) in cards" :key="index" class="w-full h-full flex-shrink-0">
						...
						<div v-else-if="index === 2" class="w-full h-full flex flex-col px-4 pt-4 pb-6 bg-[#F9FAFD]">
							...
							<div id="main-chart-container"
								class="w-full h-[280px] mb-4 bg-white rounded-lg px-4 pt-4 pb-6"></div>
							<!-- 底部数据面板 -->
							<div class="grid grid-cols-3 gap-4 h-[300px]">
								...
								<div class="bg-white rounded-lg overflow-auto shadow-md">
									...
									<div class="px-3 pb-3">
										<table class="w-full text-xs">
											...
											<tbody>
												<tr v-for="(row, rowIndex) in tableData.left" :key="rowIndex"
													class="border-b">
													...
													<td class="py-1">
														<div v-if="row.value"
															:class="row.value === 'green' ? 'w-9 h-3 bg-green-500 rounded' : 'w-6 h-3 bg-gray-400 rounded'">
														</div>
													</td>
												</tr>
											</tbody>
								...
								<div class="bg-white rounded-lg overflow-auto shadow-md">
									...
									<div class="px-3 pb-3">
										<table class="w-full text-xs">
											...
											<tbody>
												<tr v-for="(row, rowIndex) in tableData.middle" :key="rowIndex"
													class="border-b">
													<td class="py-1">{{ row.impressions }}</td>
													<td class="py-1">{{ row.clicks }}</td>
													<td class="py-1">{{ row.ctr }}</td>
								...
								<div class="bg-white rounded-lg p-3 overflow-auto shadow-md">
									...
									<table class="w-full text-xs">
										...
										<tbody>
											<tr v-for="(row, rowIndex) in tableData.right" :key="rowIndex"
												class="border-b">
												<td class="py-1">{{ row.impressions }}</td>
												<td class="py-1">{{ row.clicks }}</td>
												<td class="py-1">{{ row.ctr }}</td>
						...
						<div v-else class="grid grid-cols-4 gap-4 w-full h-full">
							<div v-for="(item, itemIndex) in slides[index]" :key="itemIndex"
		...
		<div class="block md:hidden mt-4">
			<div ref="mobileCardsContainer" class="overflow-x-auto overflow-y-hidden pb-4 -mx-4 px-4"
				...
				<div class="flex gap-4" style="width: max-content;">
					<div v-for="(card, index) in cards" :key="index" @click="activeIndex = index"
						...
						:class="activeIndex === index
							? 'bg-[#3A82F9] text-white border-[#3A82F9]'
							: 'bg-white text-gray-900 border-gray-200'" style="scroll-snap-align: start;">
		...
		<div class="flex justify-center gap-2 mt-4">
			<button v-for="(card, index) in cards" :key="index" @click="activeIndex = index"
				...
				:class="activeIndex === index ? 'bg-[#3A82F9] w-8' : 'bg-gray-300'"></button>
		</div>
	</div>
</template>
